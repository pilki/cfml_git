...
let check_nongen_scheme env = function
    Tstr_value(rec_flag, fvs, pat_exp_list) ->
      List.iter
        (fun (pat, exp) ->
...
and type_structure anchor env sstr scope =
  ...
  let rec type_struct env sstr =
    ...
    | {pstr_desc = Pstr_open lid; pstr_loc = loc} :: srem ->
        (* arthur changed 
        --> amusant, Tstr_open est défini mais toujours ignoré...
        ...
        *)
        ...
        let newenv = Env.open_signature path sg env in
        let (str_rem, sig_rem, final_env) = type_struct newenv srem in
        (Tstr_open path :: str_rem, sig_rem, final_env)
