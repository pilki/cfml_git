...
let rec type_exp env sexp =
  ...
  | Pexp_let(rec_flag, spat_sexp_list, sbody) ->
      ...
      let fvs = close_hook() in 
      let body = type_exp new_env sbody in
      re {
        exp_desc = Texp_let(rec_flag, fvs, pat_exp_list, body);
        exp_loc = sexp.pexp_loc;
...
and type_argument env sarg ty_expected' =
  ...
  | {desc = Tarrow("",ty_arg,ty_res,_); level = lv}, _ ->
      ...
      failwith "On the fly let-expansion for side effects is currently not supported";
      (*let let_pat, let_var = var_pair "let" texp.exp_type in
      let fvs = [] in  FVSTODO with effects
      re { texp with exp_type = ty_fun; exp_desc =
           Texp_let (Nonrecursive, fvs, [let_pat, texp], func let_var) }*)
      end
  | _ ->
      type_expect env sarg ty_expected
